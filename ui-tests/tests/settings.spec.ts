...
test.describe('MAAP Jupyter Server Extension Settings Test', () => {
  ...
  test('Should retrieve MAAP params from environment then update JupyterLab settings', async ({ page }) => {
    // Wait for JupyterLab to be fully loaded
    await page.waitForSelector('#jupyterlab-splash', { state: 'detached' });

    // Call the get-maap-params endpoint
    const response = await page.request.get('/maap-jupyter-server-extension/get-maap-params');
    ...
    const resp = await page.request.put(`/lab/api/settings/${PLUGIN_ID}`, {
      data: { raw: JSON.stringify({ maapApiUrl: MAAP_API_URL, maapToken: MAAP_PGT_TOKEN }) }
    });
    ...
    const getSettingsResp = await page.request.get(`/lab/api/settings/${PLUGIN_ID}`);
  ...
  test('User manually updates MAAP settings via Settings Editor UI', async ({ page }) => {
      await page.waitForSelector('#jupyterlab-splash', { state: 'detached' });
      ...
      const openMenu = page.locator('.lm-Menu').filter({ has: page.getByText('Settings Editor') });
      ...
      const apiUrlInput = page.locator('[id="jp-SettingsEditor-maap-jupyter-server-extension:plugin_maapApiUrl"]');
      const tokenInput = page.locator('[id="jp-SettingsEditor-maap-jupyter-server-extension:plugin_maapToken"]');
